/* ============================================
   CANZO - App Pages JavaScript
   Sidebar toggle, cart persistence, search, interactions
   ============================================ */

function selectRole(page, role) {
    console.log('selectRole called → page:', page, 'role:', role);
    const selector = document.getElementById(page + 'RoleSelector');
    console.log('selectRole → selector found:', !!selector);
    if (!selector) return;
    selector.querySelectorAll('.role-btn').forEach(b => b.classList.remove('active'));
    const targetBtn = selector.querySelector(`button[data-role="${role}"]`);
    console.log('selectRole → target button found:', !!targetBtn);
    if (targetBtn) targetBtn.classList.add('active');
    localStorage.setItem('canzo_role', role);
    console.log('selectRole → localStorage canzo_role set to:', localStorage.getItem('canzo_role'));
}
...
function handleRegister(e) {
    e.preventDefault();
    const role = localStorage.getItem('canzo_role') || 'student';
    console.log('handleRegister → Role:', role);
    localStorage.setItem('canzo_user_role', role);
    localStorage.setItem('canzo_user_name', document.getElementById('name')?.value || 'User');
...
const MenuSystem = {
    STORAGE_KEY: 'canzo_menu',
    SEED_DATA: [
        { id: 'biryani', name: 'Chicken Biryani', description: 'Aromatic basmati rice with tender chicken, saffron, and spices', price: 145, category: 'biryani', image: 'assets/images/biryani.jpg', inStock: true },
        { id: 'burger', name: 'Veg Burger', description: 'Crispy veg patty with fresh lettuce, tomato, and special sauce', price: 80, category: 'short-bites', image: 'assets/images/burger.jpg', inStock: true },
        { id: 'paneer-tikka', name: 'Paneer Tikka', description: 'Marinated cottage cheese grilled with peppers and onions', price: 120, category: 'short-bites', image: 'assets/images/paneer-tikka.jpg', inStock: true },
        { id: 'salad', name: 'Fresh Salad', description: 'Mixed greens, cherry tomatoes, cucumber with vinaigrette', price: 65, category: 'short-bites', image: 'assets/images/salad.jpg', inStock: true },
        { id: 'pastry', name: 'Chocolate Pastry', description: 'Rich chocolate cream layered pastry', price: 55, category: 'pastry', image: 'assets/images/pastry.jpg', inStock: true },
        { id: 'smoothie', name: 'Mango Smoothie', description: 'Fresh mango blended with yogurt and honey', price: 75, category: 'juices', image: 'assets/images/smoothie.jpg', inStock: true },
        { id: 'dosa', name: 'Masala Dosa', description: 'Crispy rice crepe with spiced potato filling, sambar and chutney', price: 70, category: 'maggie', image: 'assets/images/dosa.jpg', inStock: true },
        { id: 'fried-rice', name: 'Veg Fried Rice', description: 'Wok-tossed rice with mixed vegetables and soy sauce', price: 95, category: 'fried-rice', image: 'assets/images/fried-rice.jpg', inStock: true },
    ],
    load() {
        try {
            const data = localStorage.getItem(this.STORAGE_KEY);
    ...
    add(item) {
        ...
        item.id = item.id || `${item.name.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Role-based navigation redirect (only if user is already logged in with a known role)
    const currentRole = localStorage.getItem('canzo_user_role');
    if (currentRole) {
        ...
        if (currentRole === 'canteen' && studentPages.includes(currentPage)) {
            console.log('[Redirect] Canteen user on student page → canteen-dashboard.html');
            window.location.href = 'canteen-dashboard.html';
        } else if (currentRole === 'student' && canteenPages.includes(currentPage)) {
            console.log('[Redirect] Student user on canteen page → dashboard.html');
            window.location.href = 'dashboard.html';
        }
    }

    // Update sidebar based on role
    if (currentRole === 'canteen') {
        const sidebarNav = document.querySelector('.app-nav');
        if (sidebarNav && !sidebarNav.querySelector('[href="canteen-dashboard.html"]')) {
            sidebarNav.innerHTML = `
                <a href="canteen-dashboard.html" class="app-nav-link${currentPage === 'canteen-dashboard.html' ? ' active' : ''}">
                    <svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                    Dashboard
                </a>
                <a href="canteen-menu.html" class="app-nav-link${currentPage === 'canteen-menu.html' ? ' active' : ''}">
                ...
                <a href="canteen-orders.html" class="app-nav-link${currentPage === 'canteen-orders.html' ? ' active' : ''}">
                    <svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                    Orders
                </a>
                <a href="canteen-analytics.html" class="app-nav-link${currentPage === 'canteen-analytics.html' ? ' active' : ''}">
                    <svg viewBox="0 0 24 24"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
                    Analytics
                </a>
                <div class="app-nav-divider"></div>
                <a href="canteen-settings.html" class="app-nav-link${currentPage === 'canteen-settings.html' ? ' active' : ''}">
        ...
        // Update user info for canteen
        const userName = document.querySelector('.app-user-name');
        const userRole = document.querySelector('.app-user-role');
        if (userName) userName.textContent = localStorage.getItem('canzo_canteen_name') || 'Main Canteen';
        if (userRole) userRole.textContent = 'Canteen Admin';
    } else {
        // Update student user info
        const userName = document.querySelector('.app-user-name');
        const userRole = document.querySelector('.app-user-role');
        if (userName) userName.textContent = localStorage.getItem('canzo_user_name') || 'Ganesh';
        if (userRole) userRole.textContent = localStorage.getItem('canzo_user_email') || 'EASA College';
    }
    ...
    // Role selector visual state
    function initRoleSelector(selector) {
        if (!selector) return;
        const savedRole = localStorage.getItem('canzo_role') || 'student';
        selector.querySelectorAll('.role-btn').forEach(b => {
            b.classList.toggle('active', b.dataset.role === savedRole);
        });
    }

    initRoleSelector(document.getElementById('loginRoleSelector'));
    initRoleSelector(document.getElementById('registerRoleSelector'));
    ...
    // Sync orders between student and canteen
    syncOrders();
});

const OrderSystem = {
    ORDERS_KEY: 'canzo_orders',
    load() {
        try {
...
function syncOrders() {
    window.addEventListener('storage', (e) => {
        if (e.key === OrderSystem.ORDERS_KEY) {
            location.reload();
        }
    });
}
...
function initMenuPage() {
    ...
    // Build category tabs from menu data
    const menuItems = MenuSystem.load();
    const categories = ['all', ...new Set(menuItems.map(i => i.category))];

    function renderCategories() {
        if (categoriesEl) {
            categoriesEl.innerHTML = categories.map((cat, i) =>
    ...
    function renderMenuItems() {
        menuContainer.innerHTML = menuItems.filter(i => i.inStock).map(item => `
        ...
        // Bind add-to-cart
        menuContainer.querySelectorAll('.menu-item-add').forEach(btn => {
    ...
    // Sync: reload menu when changed from admin
    window.addEventListener('storage', (e) => {
        if (e.key === MenuSystem.STORAGE_KEY) {
            menuItems.length = 0;
            menuItems.push(...MenuSystem.load());
            renderCategories();
            renderMenuItems();
        }
    });
...
function initCartPage() {
    ...
    function updateSummary() {
        ...
        if (subtitleEl) subtitleEl.textContent = `${Cart.count()} item${Cart.count() !== 1 ? 's' : ''} from Main Canteen`;
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', () => {
            ...
            const order = {
                studentName: localStorage.getItem('canzo_user_name') || 'Ganesh',
                studentEmail: localStorage.getItem('canzo_user_email') || '',
            ...
            const savedOrder = OrderSystem.add(order);
            Cart.clear();
            renderCart();
...
function initDashboardPage() {
    ...
    const userName = document.querySelector('.app-user-name');
    const userRole = document.querySelector('.app-user-role');
    if (userName) userName.textContent = localStorage.getItem('canzo_user_name') || 'Student';
    if (userRole) userRole.textContent = localStorage.getItem('canzo_user_email') || 'EASA College';

    // Update stats from real orders
    const orders = OrderSystem.load();
    ...
    const totalSpent = orders.filter(o => o.status === 'picked').reduce((sum, o) => sum + o.total, 0);
...
function initCanteenDashboard() {
    ...
    function renderLiveQueue() {
        const orders = OrderSystem.load().filter(o => o.status === 'pending' || o.status === 'accepted' || o.status === 'preparing');
        liveQueue.innerHTML = '';

        if (orders.length === 0) {
            liveQueue.innerHTML = '<p class="canteen-live-empty">No active orders</p>';
            return;
        }
        ...
        orders.forEach(order => {
            ...
            div.innerHTML = `
                <div class="live-order-header">
                    <span class="live-order-id">${order.id}</span>
        ...
        liveQueue.querySelectorAll('.live-order-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                OrderSystem.updateStatus(orderId, statusMap[action]);
                renderLiveQueue();
            });
        });
    }
    ...
    renderLiveQueue();
    window.addEventListener('storage', (e) => {
        if (e.key === OrderSystem.ORDERS_KEY) renderLiveQueue();
    });

    // Stats cards
    const allOrders = OrderSystem.load();
    ...
    // Canteen name in header
    const settings = CanteenSystem.load();
    const canteenNameEl = document.querySelector('.app-header-subtitle');
    if (canteenNameEl) canteenNameEl.textContent = `${settings.name} · ${settings.location}`;
}
...
function initCanteenMenu() {
    ...
    function renderMenu() {
        ...
        list.querySelectorAll('.menu-management-item-toggle input').forEach(toggle => {
            toggle.addEventListener('change', function() {
                ...
                const item = MenuSystem.toggleStock(id);
                this.closest('.menu-management-item-toggle').querySelector('.menu-management-item-toggle-label').textContent = item.inStock ? 'In Stock' : 'Out of Stock';
            });
        });
        ...
        list.querySelectorAll('.menu-management-item-delete').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (itemEl) {
                    ...
                    itemEl.style.transform = 'translateX(-20px)';
                    itemEl.style.transition = 'all 0.3s ease';
                    setTimeout(() => { MenuSystem.remove(id); renderMenu(); }, 300);
                }
            });
        });
    }
    ...
    if (form) {
        form.addEventListener('submit', e => {
            ...
            if (editId) {
                MenuSystem.update(editId, { name, description: desc, price, category, image: imgSrc });
            } else {
                MenuSystem.add({ name, description: desc, price, category, image: imgSrc });
            }
    ...
    renderMenu();
    window.addEventListener('storage', (e) => {
        if (e.key === MenuSystem.STORAGE_KEY) renderMenu();
    });
}
...
function initCanteenOrders() {
    ...
    function renderOrdersTable() {
        ...
        tableBody.querySelectorAll('.canteen-order-action-select').forEach(select => {
            select.addEventListener('change', function() {
                ...
                OrderSystem.updateStatus(orderId, newStatus);
                renderOrdersTable();
            });
        });
    }
    ...
    renderOrdersTable();
    window.addEventListener('storage', (e) => {
        if (e.key === OrderSystem.ORDERS_KEY) renderOrdersTable();
    });
    ...
    function applyFilters() {
        ...
        tableBody.querySelectorAll('.canteen-orders-table-row').forEach(row => {
            const rowStatus = row.className.includes('--received') ? 'received' :
                             row.className.includes('--preparing') ? 'preparing' :
                             row.className.includes('--ready') ? 'ready' :
                             row.className.includes('--picked') ? 'picked' :
                             row.className.includes('--cancelled') ? 'cancelled' :
                             row.className.includes('--accepted') ? 'accepted' :
                             row.className.includes('--pending') ? 'pending' : '';
...
function initCanteenSettings() {
    ...
    if (autoRejectToggle) autoRejectToggle.checked = settings.autoReject;
    if (autoRejectTimeInput) {
        autoRejectTimeInput.value = settings.autoRejectTime;
        autoRejectTimeInput.disabled = !settings.autoReject;
    }
    if (maxOrdersInput) maxOrdersInput.value = settings.maxOrders;

    if (autoRejectToggle) {
        autoRejectToggle.addEventListener('change', function() {
            settings.autoReject = this.checked;
            if (autoRejectTimeInput) autoRejectTimeInput.disabled = !this.checked;
            CanteenSystem.save(settings);
        });
    }
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', function() {
            settings.name = nameInput?.value || settings.name;
            settings.location = locationInput?.value || settings.location;
            if (autoRejectToggle) settings.autoReject = autoRejectToggle.checked;
            if (autoRejectTimeInput) settings.autoRejectTime = parseInt(autoRejectTimeInput.value) || 5;
            if (maxOrdersInput) settings.maxOrders = parseInt(maxOrdersInput.value) || 20;
            CanteenSystem.save(settings);
...
function initCanteenAnalytics() {
    ...
    function renderAnalytics() {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayOrders = orders.filter(o => new Date(o.createdAt).toDateString() === dateStr && o.status === 'picked');
        ...
        if (lowDemandList) {
            ...
            orders.forEach(o => o.items.forEach(i => {
                itemOrderCount[i.name] = (itemOrderCount[i.name] || 0) + i.qty;
            }));
    ...
    renderAnalytics();
    window.addEventListener('storage', (e) => {
        if (e.key === OrderSystem.ORDERS_KEY) renderAnalytics();
    });
}
...
function initStudentOrders() {
    ...
    function renderOrders() {
        ...
        const historyOrders = orders.filter(o => ['picked', 'cancelled'].includes(o.status));
        ...
        if (activeOrders.length === 0) {
            ...
            activeOrders.forEach(order => {
                const statusIcons = { pending: 'clock', accepted: 'check', preparing: 'loader', ready: 'check-circle' };
                ...
                div.innerHTML = `
                    ...
                    <div class="order-card-info">
                        ...
                        <p class="order-card-time">${new Date(order.createdAt).toLocaleString()} · ${order.slot}</p>
                    </div>
                ...
                historyOrders.forEach(order => {
                    ...
                    div.innerHTML = `
                        ...
                        <div class="order-card-info">
                            ...
                            <p class="order-card-id">${order.id}</p>
                            <p class="order-card-items">${order.items.map(i => `${i.qty}x ${i.name}`).join(', ')}</p>
                            <p class="order-card-time">${new Date(order.createdAt).toLocaleString()}</p>
                        </div>
                        <div class="order-card-total">
    ...
    renderOrders();
    window.addEventListener('storage', (e) => {
        if (e.key === OrderSystem.ORDERS_KEY) renderOrders();
    });
}
